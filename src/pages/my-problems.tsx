...
const MyProblems: React.FC = () => {
  ...
  const renderProblemCards = (status: 'all' | 'pending' | 'resolved') => {
    const filteredProblems = problems.filter(p => 
      status === 'all' || 
      (status === 'resolved' ? p.resolved : !p.resolved)
    );
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold">
                    {renderText(problem.Problem_Category)} Issue
                    {problem.Location && ` at ${renderText(problem.Location)}`}
                    {problem.class_No && ` ${renderText(problem.class_No)}`}
                  </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <p>{renderText(problem.Reporter_Type)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Impact</p>
                    <p>{renderText(problem.Impact_Scope)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Pattern</p>
                    <p>{renderText(problem.Occurrence_Pattern)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Status</p>
                    <p>{problem.resolved ? 'Resolved' : 'Pending'}</p>
                  </div>
                </div>
          ...
          {['all', 'pending', 'resolved'].map(tab => (
            <TabsContent key={tab} value={tab}>
              <div className="space-y-4">
                {loading ? renderLoadingSkeletons() : renderProblemCards(tab as 'all' | 'pending' | 'resolved')}
              </div>
            </TabsContent>
          ))}
        </Tabs>
